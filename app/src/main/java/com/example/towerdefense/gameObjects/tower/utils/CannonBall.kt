...
class CannonBall(private var circle: Circle, private var damage: Int, health: Int, private val context: GameActivity) : Projectile, Drawable, Movable, Positionable {
    ...
    private val texture: Bitmap = BitmapFactory
        .decodeResource(context.gameView()!!.resources, R.drawable.cannon_bullet)
    private val textureResized: Bitmap =
        Bitmap.createScaledBitmap(texture, circle.radius.toInt() * 2,
            circle.radius.toInt() * 2, false)
    ...
    override fun update() {
        ...
        context.gameView()!!.surfaceView.enemies.forEach { enemy ->
